...
import com.skymob.cryptoappexemple.presentation.util.truncate


class CurrenciesAdapter(
    private val onItemClick: (Crypto) -> Unit
) : RecyclerView.Adapter<CurrenciesAdapter.CryptoViewHolder>() {
    ...
    class CryptoViewHolder(
        ...
        fun bind(item: Crypto) {
            with(binding) {
                ...
                // Define os textos
                txtName.text = item.name.truncate(11)
                txtdescription.text = item.symbol

                // Formata o valor do preço

                itemValue.text = "${item.price.roundToTwoDecimalPlaces()} $"

                // Formata o valor da variação percentual
                val percent = item.percentChange24h.roundToTwoDecimalPlaces()
